...
Abaixo está o código-fonte completo que resolve o problema utilizando a **Stream API** do Java para realizar a filtragem de forma declarativa e eficiente.
...
public class FiltroProdutos {
    public static void main(String[] args) {
        ...
        // Lista para armazenar os produtos do inventário
        List<Produto> produtos = new ArrayList<>();

        // Lê os dados individuais de cada produto
        for (int i = 0; i < n; i++) {
        ...
        // Lê os limites da faixa de preço
        double precoMin = scanner.nextDouble();
        double precoMax = scanner.nextDouble();

        // Filtragem dos produtos utilizando a Stream API do Java
        List<Produto> produtosFiltrados = produtos.stream()
             .filter(p -> p.preco >= precoMin && p.preco <= precoMax)
             .toList();

        // Exibe os produtos que atendem ao critério estabelecido
